classdef ValueAndError
    ...
    methods (Access = public  , Hidden)
        ...
        function varargout = subsref(objIn,S) 
            %overload idnexing:
            currentStruct = S(1);
            if length(S)>1
                nextStruct = S(2:end);
            ...
            Values = objIn.Value;
            Errors = objIn.Error;            
            Type   = string(currentStruct.type);
                        
            % Choose correct method:
            if Type == "()"                
                indices = currentStruct.subs{1};
                output = ValueAndError( Values(indices) , Errors(indices) );
            else
                % if a function with input, use that function:
                [varargout{1:nargout}] = builtin('subsref',objIn,S);
                return
            end
            
            % See if we need to go recuresively:
            if ~isempty( nextStruct )
                output = subsref(output,nextStruct);
            end
            varargout{1} = output;
    ...
    methods (Static)
        ...
        function ValueAndErrorOutput = fromFunction( Function , ValueAndErrorInputs)
           ...
           if LengthLongestGivenInput>1 && any(NumGivenInputs_i<LengthLongestGivenInput)
               ...
               indices_need_multiplication = assertColVec(indices_need_multiplication).'; % assert row vector
           ...
           % Itterate over all symbolic inputs to compute derivatives:
             for j = 1:NumSymbolicInputVariables
                 ...
                 % show progress:
                 progressPirnt(j,0,NumSymbolicInputVariables,LengthLongestGivenInput)
                 ...
                 for i = 1:LengthLongestGivenInput                                          
                     ...
                     % show progress:
                     progressPirnt(j,i,NumSymbolicInputVariables,LengthLongestGivenInput)
...
function vec = assertColVec(vec)
    ...
    elseif height == 1 && width == 1 
        ...
        error("A vector is expected, instead got something of size " + string( size(vec) )  );
    end
...
function progressPirnt(x,y,numX,numY )
    ...
    lengthOfNumbers = strlength(string(totalProgress));
    lengthStr       = string(lengthOfNumbers);
    ...
    formattingStr = "ValueAndError: Computation %"+lengthStr+"d of %"+lengthStr+"d ";
    nbytes = fprintf(formattingStr, currentProgress,totalProgress);    
